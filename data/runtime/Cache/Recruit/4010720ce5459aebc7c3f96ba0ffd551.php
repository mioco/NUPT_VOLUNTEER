<?php if (!defined('THINK_PATH')) exit(); $ajax = @$_REQUEST['ajax']; ?>
...
<div class="container">
    ...
    <div class="term-bar">
        ...
        <div id="time_type">
            <span>时间类型</span>
            <a href="<?php echo u('index',array('id'=>'3'));?>">全部活动</a>
            <a value="1">长期活动</a>
            <a value="0">临时活动</a>
        </div>
        <div id="term">
            <span>服务类型</span>
            <?php if(is_array($terms)): foreach($terms as $key=>$te): ?><!-- <?php echo u('list/index?id='.$te['term_id']);?> -->
                <a id="term-item-<?php echo ($te["term_id"]); ?>" value="<?php echo ($te["term_id"]); ?>" target="main"><span><?php echo ($te["name"]); ?></span></a><?php endforeach; endif; ?>
    ...
    <div class="order-bar">
        <a href="<?php echo u('recruit/recruit/index?id=3');?>">最新</a>
        <a href="">时长</a>
        <a href="">热度</a>
    </div>
    <div class="list-container">
        <?php $smeta=json_decode($vo['smeta'],true); ?>
        <?php if(is_array($posts)): $i = 0; $__LIST__ = $posts;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="post-box">
            <div class="tc-gridbox">
                ...
                <div class="body">
                    <h3><a href="<?php echo leuu('portal/article/index',array('id'=>$vo['tid']));?>"><?php echo ($vo["post_title"]); ?></a></h3>
                    <p>活动地点：<?php echo ($vo["post_address"]); ?></p>
                    <p>活动发起：<?php echo ($vo["user_login"]); ?></p>
                    <div class="timebox">
                        <p>报名截止：</p>
                        <p><div class="time"><div class="pass_time" id="<?php echo ($vo["tid"]); ?>" style="width:<?php echo ($vo["time_width"]); ?>%"></div></div></p>
                        <p class="residue"><?php echo ($status[$vo['active_status']]); ?></p>
                    </div>
                </div>
                <div class="footer">
    ...
    <hr>
    <input class="seachUrl" value="<?php echo u('recruit/recruit/listIndex');?>" type="hidden">
</div>
<br><br><br>
<!-- Footer
      ================================================== -->
    ...
    $(function(){
        $('.term-bar').on('click','a',function(){
            var variate = $(this).parent().attr('id'),
                ...
                time_limit = '',
                active_status = '',
                // check = url.indexOf(vari);
                check = url.indexOf(variate);
            // if (check>0) {
            //     console.log('data:'+data[variate]);
            //     console.log('value:'+value)
            //     var reg = RegExp(value),
            //         variate = data[variate].replace(reg, '');
            // }else{
                // data[variate] = value +','+ data[variate];
            // }
            if (check >= 0) {
