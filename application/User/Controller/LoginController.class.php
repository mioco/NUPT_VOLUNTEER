...
hello;
		$content = str_replace(array('http://#link#','#username#'), array($url,$user_number),$template);
    ...
    function dologin(){
    	...
    	// if(strpos($user_number,"@")>0){//邮箱登陆
    	// 	$where['user_email']=$user_number;
    	// }else{
    	// 	$where['user_login']=$user_number;
    	// }
    	...
    	if($ucenter_syn){
    		...
    		list($uc_uid, $user_number, $password, $email)=uc_user_login($user_number, $password);
    		...
    		if($uc_uid>0){
    			...
    			switch ($uc_uid){
    				case "-1"://用户不存在，或者被删除
    					if($result){//本应用已经有这个用户
    						if($result['user_pass'] == sp_password($password)){//本应用已经有这个用户,且密码正确，同步用户
    							$uc_uid2=uc_user_register($user_number, $password, $result['user_email']);
    							if($uc_uid2<0){
    				...
    				case -2://密码错
    					if($result){//本应用已经有这个用户
    						if($result['user_pass'] == sp_password($password)){//本应用已经有这个用户,且密码正确，同步用户
    							$uc_user_edit_status=uc_user_edit($user_number,"",$password,"",1);
    							if($uc_user_edit_status<=0){
    								$this->error("登陆错误！");
    							}
    							list($uc_uid2)=uc_get_user($user_number);
    							$uc_uid=$uc_uid2;
